...
export default function CrudEmployee() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8892/admin/getallcustomer")
      .then((response) => {
  ...
  const createPost = () => {
    if (!validate()) return;
    axios
      .post("http://localhost:8892/admin/addcustomer", {
        name,
        username,
        email,
        phone,
  ...
  const updatePost = (post) => {
    if (!validate()) return;
    axios
      .put(`http://localhost:8892/admin/updatecustomer/${post.id}`, post)
      .then((response) => {
        axios
          .get("http://localhost:8892/admin/getallcustomer")
          .then((response) => {
          ...
          .catch((error) => {
            console.error(
              "There was an error fetching the updated posts!",
              error
            );
          });
  ...
  const deletePost = (id) => {
    axios
      .delete(`http://localhost:8892/admin/deletecustomer/${id}`)
      .then(() => {
  ...
  return (
    <>
     <div className="container-fluid">
      <h1 className="mt-4">Customer Management Application</h1>
      <div className="row">
            ...
            <div className="card-body">
              <form>
                ...
                <div className="form-group">
                  ...
                  />
                  {error.balance && <p className="text-danger">{error.balance}</p>}
                </div>
                <button
                  className="btn btn-primary"
                  onClick={handleSaveClick}
                >
                  {editingPost ? "Update Customer" : "Add Customer"}
                </button>
              </form>
            </div>
          </div>
